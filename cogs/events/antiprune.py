    ...
    async def _handle(self, member: discord.Member):
        try:
            ...
            if getanti(guild.id) == "off":
                return
            entry = await self._get_recent_entry(guild, discord.AuditLogAction.member_prune)
            if not entry:
                return
            if self._is_safe(entry.user.id, guild, data["whitelisted"], data.get("wlrole")):
                return
            reason = f"AntiNuke: unauthorized prune by {entry.user}"
            asyncio.create_task(self._punish(guild, entry.user.id, data["punishment"], reason))
            asyncio.create_task(self._send_log(
                guild, self._log_embed("Member Prune Detected", entry.user, data["punishment"])
            ))
        except Exception as err:
            log.error("antiprune: %s", err)


async def setup(client: Dilbar):
    await client.add_cog(antiprune(client))
