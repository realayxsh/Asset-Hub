...
class afk(commands.Cog):
        ...
        if message.mentions:
            for user_mention in message.mentions:
                if afk[f'{user_mention.id}']['AFK'] == 'True':
                    ...
                    wl = discord.Embed(description=f'**{str(user_mention)}** went AFK <t:{ok}:R> : {reason}', color=0x2f3136)
                ...
                if afk[f'{message.author.id}']['AFK'] == 'True':
                    ...
                    if afk[f'{user_mention.id}']['AFK'] == 'True':
                        ...
                        reason = afk[f'{user_mention.id}']['reason']
                        ok = afk[f'{user_mention.id}']['time']
                        wlt = discord.Embed(description=f'**{str(user_mention)}** went AFK <t:{ok}:R> : {reason}', color=0x2f3136)
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if 'discord.gg' in reason:
            emd = discord.Embed(description=f"You can't Advertise in Afk Reason", color=0x2f3136)
        ...
        if '.gg/' in reason:
            eme = discord.Embed(description=f"You can't Advertise in Afk Reason", color=0x2f3136)
